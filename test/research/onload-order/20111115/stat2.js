...
fs.readFileSync('data2.txt', 'utf8').split('\n').forEach(function(line) {
  ...
  // onload 抢先
  if (s.indexOf('c') >= 0 && s.indexOf('c') < s.indexOf('C') ||
      s.indexOf('d') >= 0 && s.indexOf('d') < s.indexOf('D')) {
    save(16);
  }

  var L = s.replace(/[CD]/g, '');
  var U = s.replace(/[cd]/g, '').toLowerCase();
...
fs.writeFileSync('stat2.txt', out, 'utf8');
